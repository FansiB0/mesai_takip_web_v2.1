              ...
              <div className="bg-white dark:bg-gray-700 rounded-lg shadow p-6">
                <div className="flex flex-col lg:flex-row gap-4">
                  ...
                  <button
                    onClick={async () => {
                      try {
                        const csvContent = await logService.exportLogs({
                          level: logFilters.level as LogLevel || undefined,
                          category: logFilters.category as LogCategory || undefined,
                          startDate: logFilters.startDate ? new Date(logFilters.startDate) : undefined,
                          endDate: logFilters.endDate ? new Date(logFilters.endDate) : undefined
                        });
                        ...
                        showSuccess('Loglar dışa aktarıldı');
                      } catch (error) {
                        showError('Loglar dışa aktarılırken hata oluştu');
                      }
                    }}
                    className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center space-x-2"
                  >
                  ...
                  <button
                    onClick={async () => {
                      if (confirm('30 günden eski logları arşivlemek istediğinizden emin misiniz?')) {
                        try {
                          const archivedCount = await logService.archiveLogs(30);
                          showSuccess(`${archivedCount} adet log arşivlendi`);
                          loadLogs(); // Logları yenile
                        } catch (error) {
                          showError('Loglar arşivlenirken hata oluştu');
                        }
                      }
                    }}
