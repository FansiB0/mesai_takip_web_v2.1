...
export const logService = {
  ...
  async getFilteredLogs(filters: {
    ...
    try {
      ...
      if (filters.searchTerm) {
        query = query.or(`message.ilike.%${filters.searchTerm}%,user_email.ilike.%${filters.searchTerm}%,user_id.ilike.%${filters.searchTerm}%`);
      }
      ...
      // Supabase başarısız olursa local storage'dan filtrele
      const allLogs = this.getLogsFromLocalStorage(filters.limit || 1000);
      
      return allLogs.filter(log => {
        if (filters.level && log.level !== filters.level) return false;
        if (filters.category && log.category !== filters.category) return false;
        if (filters.userId && log.userId !== filters.userId) return false;
        if (filters.startDate && new Date(log.created_at || log.timestamp) < filters.startDate) return false;
        if (filters.endDate && new Date(log.created_at || log.timestamp) > filters.endDate) return false;
        if (filters.searchTerm) {
  ...
  async exportLogs(filters?: {
    ...
    try {
      const logs = filters 
        ? await this.getFilteredLogs(filters)
        : await this.getAllLogs(10000);

      // CSV formatına çevir
      const headers = [
        'Tarih', 'Seviye', 'Kategori', 'Mesaj', 'Kullanıcı Email', 
        'İşlem', 'IP Adres', 'Süre (ms)', 'Kaynak', 'Method'
      ];
      ...
      const csvContent = [
        ...
        ...logs.map(log => [
          ...
          `"${log.level}"`,
          `"${log.category}"`,
          `"${log.message.replace(/"/g, '""')}"`, // CSV için escape
          `"${log.userEmail || ''}"`,
          `"${log.userAction || ''}"`,
          `"${log.ipAddress || ''}"`,
          `"${log.duration || ''}"`,
  ...
  async archiveLogs(olderThanDays: number = 30): Promise<number> {
    try {
      ...
      if (logsToArchive && logsToArchive.length > 0) {
        // Arşiv tablosuna taşı (opsiyonel)
        ...
        // Arşivleme işlemini logla
        await this.createLog({
          level: 'info',
          category: 'admin',
          message: `${logsToArchive.length} adet log arşivlendi`,
          details: {
